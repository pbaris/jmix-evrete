...
The UI provides a user-friendly interface for working with the Evrete rule engine, leveraging its *Annotated Java Rules API*.
...
There are four properties available to configure the behavior of the add-on.
...
|jmix.evrete.default-facts-provider-force-inapplicable|false|Forces the `DefaultFactsProvider` to be inapplicable, regardless of the rest of the configuration.
|jmix.evrete.default-facts-provider-force-annotated-applicable|false|Forces `DefaultFactsProvider` to be applicable when `entitySelectionMode` is `annotated`.
|jmix.evrete.default-facts-provider-force-normal-inapplicable|false|Forces `DefaultFactsProvider` to be inapplicable when `entitySelectionMode` is `normal`.
|===
